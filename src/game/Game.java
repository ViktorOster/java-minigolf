...
public class Game extends Canvas implements Runnable{
	...
	public Game(){
		...
		new Window((int)WIDTH, (int)HEIGHT, "Minigolf", this);
		BufferedImageLoader loader = new BufferedImageLoader();
		cam = new Camera(0, 0);
		
		testLevel = loader.loadImage("/testLevel.png");
		mapImage= loader.loadImage("/golf1.png");
		
		handler.addObject(new Ball(40, 100, ID.Ball, handler));
	...
	public void loadImageLevel(BufferedImage image){
		...
		for(int i = 0; i < h; i++){
			for(int j = 0; j < w; j++){
				...
				if(red == 255 && green == 0 && blue == 0) handler.addObject(new Exit(i*32, j*32, ID.Exit, handler));
